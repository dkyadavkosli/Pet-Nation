...
function FavCom() {
  ...
  const getPets = async() => {
    try {
      const res  = await axios.get("/api/favourite/user/"+myUser?._id);
      setAllPets(res.data);
    } catch (err) {
  ...
  return (
    <div className="2xl:pl-24 xl:pl-20 lg:pl-16 md:pl-6 sm:pl-4 pl-2 2xl:pr-24 xl:pr-20 lg:pr-16 md:pr-6 sm:pr-4 pr-2 w-full wq:pt-6 pt-4 wq:pb-6 pb-4">
      ...
      <div className={`${allPets?.length===0?"":"hidden"} flex flex-row w-full justify-center`}>
        <div className="font-semibold">YOU DON'T HAVE PUT ANY FAVOURITE PETS</div>
      </div>
      ...
      {allPets?.map((elem)=>{
              return(
               <PetCard key={elem?._id} fav={"yes"} id={elem?.pet_id} img={elem?.image} name={elem?.pet_name} breed={elem?.breed} city={elem?.city}/>
              )
      })}
      </div>
